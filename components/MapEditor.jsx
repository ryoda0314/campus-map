...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
export default function MapEditor({ campusData }){
  const { CAMPUS, SPOTS, SPOT_CATS, AREAS, ENTRANCES, WAYPOINTS, EDGES } = campusData;
  const CAMPUS_CENTER = CAMPUS.center;
  const CAMPUS_ZOOM = CAMPUS.zoom;
  const NAV_SPOTS = useMemo(() => getNavSpots(SPOTS), [SPOTS]);
  ...
  // Navigation
  const nav=useNavigation(SPOTS, WAYPOINTS, ENTRANCES, EDGES);
  ...
  useEffect(()=>{
    ...
    if(mode==="spots"){
      SPOTS.forEach(s=>{
        ...
        m.on("dragend",()=>{map.dragging.enable();const ll=m.getLatLng();setEdits(prev=>({...prev,[s.id]:{lat:parseFloat(ll.lat.toFixed(5)),lng:parseFloat(ll.lng.toFixed(5))}}));});
    ...
    if(mode==="paths"){
      ...
      if(pathSub==="wp"){
        ...
        // Show entrances as reference dots in wp mode
        ENTRANCES.forEach((ent,idx)=>{
          const s=SPOTS.find(x=>x.id===ent.spot);
          const dot=L.divIcon({className:"",html:`<div style="width:7px;height:7px;border-radius:50%;background:#4de8b060;border:1px solid #4de8b040;transform:translate(-50%,-50%)"></div>`,iconSize:[0,0],iconAnchor:[0,0]});
          const m=L.marker([ent.lat,ent.lng],{icon:dot}).addTo(map);
        ...
        // Show entrances as reference dots in view mode
        ENTRANCES.forEach((ent,idx)=>{
          const s=SPOTS.find(x=>x.id===ent.spot);
          const dot=L.divIcon({className:"",html:`<div style="width:7px;height:7px;border-radius:50%;background:#4de8b060;border:1px solid #4de8b040;transform:translate(-50%,-50%)"></div>`,iconSize:[0,0],iconAnchor:[0,0]});
          const m=L.marker([ent.lat,ent.lng],{icon:dot}).addTo(map);
    ...
    SPOTS.forEach(s=>{
      ...
      const pos=edits[s.id]||{lat:s.lat,lng:s.lng};
      const edited=!!edits[s.id];
      const isDraggable=viewDrag;
      const icon=L.divIcon({className:"",
        html:`<div style="display:flex;align-items:center;gap:4px;padding:3px 7px 3px 5px;border-radius:8px;background:${edited?"#e8b63a":s.col};box-shadow:0 2px 8px rgba(0,0,0,.4);${isDraggable?"cursor:grab;":""}white-space:nowrap;font-family:'Inter',sans-serif;transform:translate(-50%,-50%);border:2px solid ${edited?"#fff":"transparent"}">
          <span style="width:16px;height:16px;border-radius:3px;background:rgba(255,255,255,.25);display:flex;align-items:center;justify-content:center;font-size:7px;font-weight:700;color:#fff">${s.short}</span>
          <span style="font-size:10px;font-weight:600;color:#fff">${s.label}</span></div>`,
        iconSize:[0,0],iconAnchor:[0,0],
      });
      const m=L.marker([pos.lat,pos.lng],{icon,draggable:isDraggable}).addTo(map);
      if(isDraggable){
        m.on("dragstart",()=>{map.dragging.disable();});
        m.on("dragend",()=>{map.dragging.enable();const ll=m.getLatLng();setEdits(prev=>({...prev,[s.id]:{lat:parseFloat(ll.lat.toFixed(5)),lng:parseFloat(ll.lng.toFixed(5))}}));});
      }
  ...
  },[leafletReady,mode,edits,areaEdits,areaSelSpot,pathSub,wpEdits,newWps,edgeFrom,newEdges,edgeType,deletedEdges,entEdit,newEnts,entDragEdits,spotRegType,spotRegs,navDest,navOrigin,nav.route,viewDrag]);
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:T.bg,fontFamily:"'Inter',sans-serif",color:T.txH}}>
      ...
      <header style={{display:"flex",alignItems:"center",gap:8,padding:"8px 16px",background:T.bg2,borderBottom:`1px solid ${T.bd}`,flexShrink:0,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginRight:8}}>
          <a href="/" style={{display:"flex",alignItems:"center",textDecoration:"none"}} title="キャンパス一覧">
          ...
          <span style={{fontSize:10,color:T.txD,fontWeight:500}}>{CAMPUS.name}</span>
        </div>
...
.spot-tip .leaflet-tooltip-arrow{display:none!important}
        ...
        {/* ── View mode drag toggle ── */}
        {mode==="view"&&<div style={{position:"absolute",top:10,left:10,zIndex:1000}}>
          <button onClick={()=>setViewDrag(e=>!e)} style={{padding:"5px 10px",borderRadius:6,border:`1.5px solid ${viewDrag?"#e8b63a":T.bd}`,background:viewDrag?"#e8b63a":`${T.bg2}e0`,backdropFilter:"blur(8px)",cursor:"pointer"}}>
            <span style={{fontSize:10,fontWeight:600,color:viewDrag?"#000":T.txH}}>{viewDrag?"ドラッグ編集中":"座標編集"}</span>
          </button>
        </div>}
        ...
        {/* Spot edit panel */}
        {(mode==="spots"||mode==="view")&&editCount>0&&<div style={{position:"absolute",top:10,right:10,zIndex:1000,width:260,borderRadius:10,background:`${T.bg2}f0`,backdropFilter:"blur(8px)",border:`1px solid ${T.bdL}`,boxShadow:"0 4px 16px rgba(0,0,0,.4)",padding:10,maxHeight:300,overflowY:"auto"}}>
