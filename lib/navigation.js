...
function buildGraph(SPOTS, WAYPOINTS, ENTRANCES, EDGES) {
  const nodes = {};
...
export function getNavSpots(SPOTS) {
  return SPOTS.filter(s => s.id && s.lat != null);
}

export function useNavigation(SPOTS, WAYPOINTS, ENTRANCES, EDGES) {
  const [origin, setOrigin] = useState(null);
  const [destination, setDestination] = useState(null);
  const [gpsOriginPos, setGpsOriginPos] = useState(null); // {lat, lng} — GPS由来の出発地座標

  const graph = useMemo(() => buildGraph(SPOTS, WAYPOINTS, ENTRANCES, EDGES), [SPOTS, WAYPOINTS, ENTRANCES, EDGES]);
